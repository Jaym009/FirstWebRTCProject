...
const Dashboard = () => {
  ...
  const [showReceiverDetail, setshowReceiverDetail] = useState(null);
  ...
  useEffect(() => {
    // Initialize Howl only once when component mounts
    ringtoneRef.current = new Howl({
      src: ["/ringtone.mp3"], // Make sure this path is correct
      loop: true, // Changed to true to keep ringing
      volume: 0.8, // Slightly reduced volume for better UX
  ...
  const startCall = async () => {
    try {
      ...
      setSelectedUser(showReceiverDetail._id);
      ...
      peer.on("signal", (data) => {
        ...
        socket.emit("callToUser", {
          callToUserId: showReceiverDetail._id,
          signalData: data,
          from: me,
          name: user.username,
          email: user.email,
  ...
  const handelSelectedUser = (user) => {
    if (!isOnlineUser(user._id)) {
      setUserUnavailable(true);
      setUnavailableMessage(`${user.username} is currently offline`);
      return;
    }
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
        ...
        <ul className="space-y-4 overflow-y-auto">
          {filteredUsers.map((user) => (
            <li
              ...
              } ${!isOnlineUser(user._id) ? "opacity-60" : ""}`} // Added opacity for offline users
              onClick={() => {
                if (!isOnlineUser(user._id)) {
                  setUserUnavailable(true);
            ...
            >
              <div className="relative">
                ...
                {isOnlineUser(user._id) ? (
                  ...
                  <span className="absolute top-0 right-0 w-3 h-3 bg-gray-400 border-2 border-gray-800 rounded-full shadow-lg"></span>
                )}
              </div>
          ...
          <div className="relative w-full h-screen bg-black flex items-center justify-center">
            ...
            <div className="absolute bottom-[75px] md:bottom-0 right-1 bg-gray-900 rounded-lg overflow-hidden shadow-lg">
              <video
                ...
                autoPlay
                playsInline
                className="w-32 h-14 md:w-56 md:h-52 object-cover rounded-lg"
              />
            </div>
      ...
      {showReciverDetailPopUp && showReceiverDetail && (
      ...
      {userUnavailable && (
        <div className="fixed inset-0 bg-transparent bg-opacity-30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6">
            <div className="flex flex-col items-center">
              <p className="font-black text-xl mb-2 text-red-600">
                User Unavailable
              </p>
